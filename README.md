...
Its probably easier if you have a static IP on your ISP which will get around having to update your Azure settings whenever your ISP changes your dynamic IP, you cant use DynDNS in the Azure connection setup (yet). The only way around it is to run a automated update script for your connection. The script 'AzureLNG-IPUpdate' is what i use in a Automation Powershell Runbook to update my DynDNS IP once per hour. You will need to set you DynDNS name as a variable in the runbook.
